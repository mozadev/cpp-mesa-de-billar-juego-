...
void Tiro::mover(int k) {
	gotoxy(x, y);printf(" ");
	if (y > 4) y--;
	gotoxy(x, y);printf("%c", k);
...
int main() {
	...
	list<Tiro*>tiro;  // arreglo donde guardamos a un puntero de la clase tiros
	...
	while (!gameover) {
		...
		if (_kbhit())
		{
			...
			if (tecla == 'a')
			{
				tiro.push_back(new Tiro(N.X(), N.Y()));

			}

		}
		for (it = tiro.begin();it != tiro.end();it++) {
			(*it)->mover(49);
		}
